    ...
    <b-container class="mt-5">
      <b-row>
        <b-col cols="12" md="4" lg="3">
          <div>
            <h5>Categorías</h5>
            <CategoriesProductList :parent="params.categoria" :categories="categoryList"/>
          </div>
        </b-col>
...
<script lang="ts">
  ...
  const Categories = Vue.extend({
    ...
    async asyncData({params, $api, query, route, error}) {
      try {
        const categoryData: {data: Category[]} = await $api.get(`products/categories?slug=${params.categoria}`);
