    ...
    <b-container class="mt-5">
      <b-row>
        <b-col cols="12" md="4" lg="3">
            ...
            <div>
                <h5>Categorías</h5>
                <CategoriesProductList :parent="''" :categories="categoryList"/>
            </div>
        </b-col>
        <b-col cols="12" md="8" lg="9">
          <b-row>
            <b-col v-for="(item, index) in products" :key="index" cols="6" md="4" lg="3" >
...
<script lang="ts">
  ...
  import { Category, Product } from '~/interfaces/interfaces';
    ...
    export default Vue.extend({
        ...
        watchQuery: ['page'],
        async asyncData({params, $api, query, route, error}) {
            try {
                const categoryData: {data: Category[]} = await $api.get(`products/categories?slug=${params.subcategoria}`);
                const productList: {headers: any, data: Product[]} = await $api.get(`products?category=${categoryData.data[0].id}&page=${query.page ? query.page : 1}&per_page=2`);
                const categoryList: {data: Category[]} = await $api.get(productList.data.length > 0 ? `products/categories?parent=${categoryData.data[0].id}` : `products/categories`);
                ...
                return {
                    ...
                    path: route.path,
                    params,
                    // items: Object.keys(params).map(m => { 
                    //             return {
                    //                 text: 'Admin',
                    //                 href: '#'
                    //             } 
                ...
                error({ statusCode: 500, message: 'Ocurrió algo en nuestro servidor. Vuelva a intentar.' })
            }
